...
**Build Tool:** Maven | **Arquitetura:** MVC | **Porta:** 8080
...
├── controller/
│   └── FallbackController.java          # Respostas de fallback/erro
├── service/
│   ├── JwtValidationService.java        # Validação de JWT
│   └── RateLimitService.java            # Lógica de rate limiting com Redis
├── model/
│   └── JwtClaims.java                   # Claims extraídos do token
├── exception/
│   └── GatewayExceptionHandler.java     # Handler global de erros reativos
├── config/
│   ├── GatewayRoutesConfig.java         # Rotas Spring Cloud Gateway
│   ├── SecurityConfig.java              # Spring Security reativo
│   └── CorsConfig.java                  # CORS para o frontend
└── GatewayApplication.java
```

> Nota: `messaging/` e `repository/` não se aplicam ao Gateway. O padrão MVC adapta-se ao contexto reativo mantendo a separação controller/service/model/config.
